...
import { CartItemRepository } from '../../cart_items/infraestructure/cartitem.repository.js';
...
import { ListOrdersByUserUseCase } from '../application/ListOrdersByUserUseCase.js';
import { ListAllOrdersUseCase } from '../application/ListAllOrdersUseCase.js';
...
export class OrderController {
  async index(request, response, next) {
    try {
      const useCase = new ListOrdersByUserUseCase(orderRepository);
  ...
  async store(request, response, next) {
    try {
      const useCase = new CreateOrderUseCase(orderRepository, cartItemRepository);
  ...
  async list(request, response, next) {
    try {
      const useCase = new ListAllOrdersUseCase(orderRepository);
